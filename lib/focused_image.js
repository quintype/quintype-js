...
function imgixPath(opts) {
  ...
  return "?" + _.chain(opts).map(function(value, key) {
    return key + "=" + encodeURIComponent(_.isArray(value) ? _.join(value, ",") : value);
  }).join("&").value();
}
...
FocusedImage.prototype.path = function(aspectRatio, opts) {
  ...
  return encodeURIComponent(this.slug) + imgixPath(_.extend(rectOpts, opts));
}
